...
ConnectionManager.prototype.onSocketClosed = function (hasError) {
    ...
    console.log("%s - onSocketClosed:L275  setState(DISCONNECTED) PacketQueue size: %s, PendingQueue size: %s",
        new Date(), this.packetQueue.length(), this.pendingQueue.length);
    ...
    if (self.state === STATES.CONNECTING) {
        ...
        default:
            ...
            if (!pendingPacket) {
                ...
                // );
                console.log("%s - onSocketData:L488  socket.destroy execute PacketQueue size: %s, PendingQueue size: %s",
                    new Date(), this.packetQueue.length(), this.pendingQueue.length);
            ...
            if (pendingPacket.request.header.xid !== responseHeader.xid) {
                ...
                //);
                console.log("%s - onSocketData:L503  socket.destroy execute PacketQueue size: %s, PendingQueue size: %s, pendingPacket xid = %s !== response xid = %s",
                    new Date(), this.packetQueue.length(), this.pendingQueue.length, pendingPacket.request.header.xid, responseHeader.xid);
                self.socket.destroy(); // this will trigger reconnect
                return;
            }
